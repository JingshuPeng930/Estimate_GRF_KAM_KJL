...
def build_kjl_ab03_dataloaders(
    ...
    if split_json:
        ...
        if not split_path.is_absolute() and not split_path.exists():
            for candidate in (root / split_path, Path.cwd() / split_path):
                if candidate.exists():
        ...
        noexo_dirs = sorted(root.glob("*/LG/NoExo/trial_1"))
        exo_dirs = [p for p in sorted(root.glob("*/LG/*/trial_1")) if "NoExo" not in p.parts]
